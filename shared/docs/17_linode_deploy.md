...
- first stack bring-up through `python3 -m rusaren_ops deploy`
...
By default, `python3 -m rusaren_ops deploy` now rebuilds the Godot web bundle on the host before it builds the Docker image.
...
`python3 -m rusaren_ops setup` now handles the host-side baseline:
- updates packages
...
`python3 -m rusaren_ops setup` installs Docker Engine from Docker's official apt repository, not the convenience script.
...
2. Let `python3 -m rusaren_ops setup` install a compatible Godot snap.
3. Let `python3 -m rusaren_ops deploy` run `python3 -m rusaren_ops export-web-client` on the host before the Docker build.

If you prefer to prebuild locally instead, this still works:

```bash
python3 -m rusaren_ops export-web-client --godot-bin godot4
```
...
`python3 -m rusaren_ops setup` writes `~/rusaren-config/config.env` and `~/rusaren-config/docker-compose.override.yml`.
...
`python3 -m rusaren_ops deploy` now:
...
```bash
sudo BUILD_WEB_CLIENT=0 python3 -m rusaren_ops deploy
```
